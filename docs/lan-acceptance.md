...
3. B、C 无需邀请码，并在 5 秒内直接加入默认开放群组。
4. A 另建明确启用邀请码的群组，B 使用正确邀请码加入。
5. A 公开需求，并向 B-Pi 发送服务端任务。
6. B-Pi 修改 B 的私有仓库，再把任务转交 C-Pi。
7. C-Pi 修改 C 的私有仓库，运行测试并公开最终结果。
8. 确认回答只出现一次，并能对应 A 的原始任务。
