...
DEMO_DIR="$ROOT/docs/demo/trace-relay"
FRAME_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pi-comms-trace-relay-frames.XXXXXX")"
...
for state in 0 1 2 3 4 5 6; do
  "$CHROME_BIN" --headless --no-sandbox --disable-gpu --hide-scrollbars \
    ...
    "file://$DEMO_DIR/trace-relay-demo.html#$state"
done

cp "$FRAME_DIR/state-0.png" "$DEMO_DIR/cover.png"
cp "$FRAME_DIR/state-1.png" "$DEMO_DIR/screenshots/01-private-repos-online.png"
cp "$FRAME_DIR/state-4.png" "$DEMO_DIR/screenshots/02-contract-handoff.png"
cp "$FRAME_DIR/state-6.png" "$DEMO_DIR/screenshots/03-end-to-end-trace.png"
...
"$FFMPEG_BIN" -y "${inputs[@]}" -filter_complex \
  ...
  "$DEMO_DIR/trace-relay-three-private-repos.mp4"

"$FFMPEG_BIN" -y -i "$DEMO_DIR/trace-relay-three-private-repos.mp4" -an \
  -c:v libvpx-vp9 -deadline realtime -cpu-used 8 -b:v 900k -row-mt 1 \
  "$DEMO_DIR/trace-relay-three-private-repos.webm"
