...
- [ ] 新增 TCP Broker 端点类型，明确区分监听端点和连接端点，至少包含 `host` 和 `port`。
...
- [ ] 用 TCP 探测、端口绑定和跨进程启动互斥替代 Unix 文件链接锁；替代逻辑完成前不得删除旧锁。
- [ ] 保证关闭时停止接受连接并清理全部客户端。

### 保持本机单 Broker

- [ ] 为正式运行定义固定本机端点；端口 `0` 只用于测试和显式临时实例。
- [ ] 两个 Pi Session 并发首次连接时只能成功启动一个 Broker。
- [ ] 启动者遇到 `EADDRINUSE` 后重新握手探测；确认是兼容 Broker时改为复用，不能直接失败。
- [ ] 端口被无关进程占用时返回明确错误，不能误认成 Pi Comms Broker。
- [ ] 本机自动启动逻辑必须等待 Broker 完成协议握手，不能只判断 TCP 端口已打开。
- [ ] 阶段 12 的实现 PR 不得在缺少以上行为时单独合并。
...
- [ ] 连接目标改变时停止旧连接和旧重连计时器。
- [ ] 握手确认目标是兼容的 Pi Comms Broker，而不是任意占用该端口的进程。
...
- [ ] 覆盖临时端口、固定端口占用、关闭和重启。
- [ ] 覆盖两个 Extension 并发首次启动时只产生一个 Broker。
- [ ] 覆盖端口被无关进程占用时不会复用或启动第二个 Broker。
- [ ] 保证阶段 0～11 的行为测试继续通过。
...
- [ ] 本机两个 Pi Session 能通过 loopback TCP 完成现有全部通信，包括并发首次启动。
