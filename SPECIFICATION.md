...
`protocolVersion`、`deviceId`、`sessionId`，以及首次加入使用的群组邀请码或
后续恢复使用的长期成员凭证。首次连接由 `client.welcome` 返回当前进程使用的
...
- 加入前的群组目录只返回允许附近加入的群组 ID、名称和在线 Session 数，不创建客户端身份，也不返回成员、Agent、消息或历史记录。
...
- 每个附近群组独立生成 10 位 Base32 邀请码，显示时按 `5-5` 分组。
- 完整邀请信息包含端点和群组邀请码，粘贴后直接定位群组。
- 邀请码只用于首次加入；成功后签发长期成员凭证。
- 长期成员凭证用于断线、Session 重开、Broker 重启和网络切换后的自动恢复。
- 重新生成邀请码只影响新成员，不影响已有长期成员。
- 关闭附近加入时旧邀请码失效；再次开放时生成新邀请码。
- 邀请码不自动过期，不进入 mDNS、普通日志或错误文本。
- 邀请码缺失或错误时返回明确错误并立即关闭；同一来源 60 秒内连续失败 5 次后冷却 30 秒，成功后清零。
- 正确邀请码直接加入，不增加群主审批。
- 群主可以按稳定 Session 身份移出成员；被移出成员停止重连并需要解除阻止和新邀请码才能重新加入。
...
- 用户选择附近群组后输入短邀请码；完整邀请信息可以直接定位群组。
